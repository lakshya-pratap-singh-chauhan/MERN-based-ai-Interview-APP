...
function Step1SetUp({ onStart }) {
    ...
    const handleStart = async () => {
        ...
        try {
           ...
           if (!result.data?.interviewId || !Array.isArray(result.data.questions) || result.data.questions.length === 0) {
            throw new Error("The interview could not be created. Please try again.");
           }
           console.log(result.data)
           if(userData){
           ...
           onStart(result.data)
            ...
            setError(error.response?.data?.message || error.message || "Unable to start the interview. Please try again.");
            setLoading(false)
        }
    }
    return (
